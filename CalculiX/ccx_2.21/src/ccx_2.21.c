...
int main(int argc,char *argv[])
{
  ...
  printf("************************************************************\n\n");
  printf("You are using an executable made on Thu Dec 12 23:32:13 EST 2024\n");
  fflush(stdout);
  ...
  while(istat>=0){
	...
	NNEW(edc,double,12*ndc_);
      ...
      NNEW(alcon,double,25*ntmat_*nmat);
      NNEW(nalcon,ITG,2*nmat);
      NNEW(alzero,double,nmat);

      /* conductivity */

      NNEW(cocon,double,25*ntmat_*nmat);
	...
	RENEW(ne2boun,ITG,2*infree[3]);
      ...
      RENEW(cocon,double,25*ntmat_*nmat);
