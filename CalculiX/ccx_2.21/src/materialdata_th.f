...
!
     ...
     &  pgauss(3),rhcon(0:1,ntmat_,*), i, j,
     &  shcon(0:3,ntmat_,*),rho,sph,xstiff(27,mi(1),*)
	  real*8 specht
	 
	  COMMON /THM_DATA/ specht
...
!     
!        nshcon = number of temperature dependent sp. heat
!		 
         call ident2(shcon(0,1,imat),t1l,nshcon(imat),four,id)
         if(nshcon(imat).eq.0) then
!            continue
!			sph=CPA(1)+CPA(2)*t1l
!			here specified the specht from abaqus_sub.dll through common
			sph=specht
