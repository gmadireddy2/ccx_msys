...
! This subroutine works as an example of how the Dante's subroutine needs
! to be written for allowing it to be integrated with CalculiX.
! This subroutine needs to be compiled with gfortran using Cygwin. 
! -- Guru Madireddy 08/07/2024

! -->> Abaqus UMATHT subroutine 

      subroutine abqsub(u,dudt,temp,dtemp,ntgrd,dfdg,flux,dtemdx,
     &  statev,dudg,time,dtime,predef,dpred,cmname,nstatv,
     &  props,nprops,coords,pnewdt,noel,npt,layer,kspt,kstep,kinc,
     &  vold,co,lakonl,konl,ipompc,nodempc,coefmpc,nmpc,
     ...
     &  vold(0:mi(2),*),co(3,*),coefmpc(*),cond,CPA(2),AKA(2)
      ...
      statev(2) = dtemp
      statev(3) = 0
	  ! print *, '1st Reading file with thread:', omp_get_thread_num()

      !$omp parallel
         !$omp critical
         ! Only one thread will perform the file I/O
		 ! print *, 'Reading file with thread:', omp_get_thread_num()
         ...
         ! Calculate flux and dfdg in parallel
!         !$omp do
         do i = 1, ntgrd
...
!         !$omp do
         DO i = 1, ntgrd
            DO j = 1, ntgrd
               dfdg(i, j) = 0.d0   ! Non-diagonal elements explicitly set to 0
            END DO
            dfdg(i, i) = cond      ! Set conductivity
         END DO
!         !$omp end do
      !$omp end parallel
